...
**CreateFile3** behaves exactly the same way as [CreateFile2](nf-fileapi-createfile2.md) with one exception; operations will fail if *lpFileName* is redirected via a reparse point or symbolic link. This behavior can be disabled with a new flag that can be added to the *dwFileFlags*.
